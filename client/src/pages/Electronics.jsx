...
import {
  ...
  ListProductCard,
  Components,
} from "./ElectronicsContext"; // <-- FIX: Import Components from context
...
const Electronics = () => {
  ...
  // Destructure icons from the Components export
  const { UserCircleIcon, UserIcon, CubeIcon, HeartIcon, ShoppingCartIcon } =
    Components;

  // RENDER WISHLIST PAGE (Logic simplified to use React Router, though original logic is preserved for the context of the user request)
  // NOTE: This now programmatically navigates via React Router. The original logic was kept in the context file for simplicity of this single-file output.
  const handleShowWishlist = () => {
    setIsDropdownOpen(false); // Close dropdown
    navigate("/wishlist"); // Navigate to the new Wishlist page
  };
        ...
        <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-6">
              ...
              {/* CART NAVIGATION BUTTON (Refactored to use Link) */}
              <Link
                to="/cart" // <-- CHANGE: Use Link to navigate to the new /cart route
                className="flex items-center space-x-1 text-white hover:text-orange-400 transition-colors"
              >
                <div className="relative">
                  <ShoppingCartIcon className="h-6 w-6" />
